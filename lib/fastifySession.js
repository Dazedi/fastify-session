...
function decryptSession (sessionId, options, request, done) {
  ...
  if (options.expressCompat && sessionId.substr(0, 2) === 's:') {
  ...
  if (decryptedSessionId === false) {
    ...
    options.store.get(decryptedSessionId, (err, session) => {
      ...
      if (options.expressCompat) {
        session.sessionId = decryptedSessionId;
        if (session.cookie._expires) {
          session.expires = session.cookie._expires.getTime();
        }
      }
      ...
      request.session = new Session(
        idGenerator,
        { ...cookieOpts, expressCompat: options.expressCompat },
        secret,
        session
      )
      done()
...
function onSend (options) {
  return function saveSession (request, reply, payload, done) {
    ...
    if (options.expressCompat) {
      session.cookie._expires = new Date(session.expires);
    ...
    options.store.set(session.sessionId, session, (err) => {
      ...
      reply.setCookie(
        options.cookieName,
        (options.expressCompat ? 's:' : '') + session.encryptedSessionId,
...
function ensureDefaults (options) {
  ...
  options.expressCompat = option(options, 'expressCompat', false)
  return options
}
